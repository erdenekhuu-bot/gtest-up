...
export async function PUT(req: NextRequest) {
  try {
    ...
    const list: any = document?.departmentEmployeeRole.filter(
      (item: any) => item.employee.jobPosition.jobPositionGroup.jobAuthRank <= 2
    );
    const dir: any = document?.departmentEmployeeRole.filter(
      (item: any) =>
        item.employee.jobPosition.jobPositionGroup.jobAuthRank <= 4 &&
        item.employee.jobPosition.jobPositionGroup.jobAuthRank > 2
    );
    const ceo: any = document?.departmentEmployeeRole.filter(
      (item: any) =>
        item.employee.jobPosition.jobPositionGroup.jobAuthRank <= 6 &&
        item.employee.jobPosition.jobPositionGroup.jobAuthRank > 4
    );
...
async function Demo(mobile: string) {
  const sendSms = `http://sms-special.gmobile.mn/cgi-bin/sendsms?username=${
    ...
    &password=${process.env.OTP_PASSWORD}&from=245&to=${mobile}
