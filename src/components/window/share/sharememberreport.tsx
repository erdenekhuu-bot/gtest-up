...
export function ShareMemberReport({ data }: any) {
  ...
  const onFinish: FormProps["onFinish"] = async (values) => {
    ...
    const reporttesterror = await (values.reporttesterror || []).map(
      (item: any) => {
        return {
          list: item.list,
          level: item.level,
  ...
  const handleDownloadPdf = async (id: number) => {
    setPdfLoading(id);
    try {
      const response = await fetch(`/api/download/report/${id}`);
      if (!response.ok) {
  ...
  return (
    <Form className="p-6 relative" form={mainForm} onFinish={onFinish}>
      {contextHolder}

      <div
        className="fixed"
        style={{ top: "860px", right: "40px", zIndex: 10 }}
      >
        <Button
          type="primary"
          icon={<DownloadOutlined />}
          onClick={() => handleDownloadPdf(Number(data.documentId))}
          shape="circle"
          size="large"
          style={{ boxShadow: "0 4px 6px rgba(0, 0, 0, 0.1)" }}
          loading={pdfLoading === Number(data.documentId)}
        />
      </div>
